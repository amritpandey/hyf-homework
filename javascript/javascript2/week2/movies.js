...
//1. Create an array of movies containing the movies with a short title (i define movie with only one word as short movie)

const shortTitleMovie = movies.filter((movie) => movie.title.length <= 3);
console.log(shortTitleMovie);

// 2. long movie titles
const longTitleMovie = movies.filter((movie) => movie.title.length > 50);
console.log(longTitleMovie);

//3. Count the number of movies made between 1980-1989
//using forEach
let count = 0;
const numberOfMovies = movies.forEach((movie) => {
    if (movie.year >= 1980 && movie.year <= 1989) {
        count++;
    }
});
console.log(`Movies made between 1980-1989:${count} `);

//3. using filter
const noOfMovies = movies.filter(
    (movie) => movie.year >= 1980 && movie.year <= 1989,
).length;
console.log(`Movies made between 1980-1989 :${noOfMovies} `);

//4. Adding an extra key called tag on movies
const extraTagInMovies = movies.map((movie) => {
    let tag;
    if (movie.rating >= 7) {
        tag = 'good';
...
console.log(extraTagInMovies);

//4. alternate way to add extra tag key to movies
/* const extraTagInMovies = movies.map((movie) => {
...
// 6.  Count the total number of movies with Surfer,alien & benjamin ...keywords

const totalNumberOfSurfer = movies.filter((movie) =>
    movie.title.toLowerCase().includes('surfer'),
);

const totalNumberOfAlien = movies.filter((movie) =>
    movie.title.toLowerCase().includes('alien'),
);

const totalNumberOfBenjamin = movies.filter((movie) =>
    movie.title.toLowerCase().includes('benjamin'),
);

console.log(totalNumberOfSurfer);
console.log(totalNumberOfAlien);
console.log(totalNumberOfBenjamin);

const totalNumberOfMovies =
    totalNumberOfSurfer.length +
    totalNumberOfAlien.length +
    totalNumberOfBenjamin.length;

console.log(totalNumberOfMovies);

// 7. find movie with duplicated word in the movie title
const duplicateWordMovieFinding = movies.forEach((movie) => {
    const movieTitlesInArray = movie.title.split(' ');
    movieTitlesInArray.filter((newMovie, index) => {
        if (index !== movieTitlesInArray.indexOf(newMovie)) {
            console.log(movie.title);
        }
    });
});
console.log(duplicateWordMovieFinding);

// 8.Calculate the average rating of all the movies
const averageRatingOfMovies =
    movies
        .map((movie) => movie.rating)
        .reduce((previousValue, currentValue) => previousValue + currentValue) /
    movies.length;
avgRatingCalculation = Math.round((averageRatingOfMovies * 100) / 100);

console.log(avgRatingCalculation);
